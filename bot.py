...
def check_ops():
    ...
    for op in ops:
        ...
        if hit_tp:
            ...
            with lock:
                state['tp_count']           += 1
                state['sl_streak'][op['asset']] = 0   # reset racha
        else:
            send(f'❌ SL\n\n{cfg["icon"]} {cfg["label"]}\n\nHora: {now_str()}')
...
def send_op(asset, direction, px, score):
    cfg   = ASSETS[asset]
    isBuy = direction == 'buy'
    ...
    msg = (f'{cfg["icon"]} {cfg["label"]}\n\n'
           ...
           f'Hora: {hora}')
    send(msg)
    log(f'📨 SEÑAL #{n} {tipo} {cfg["label"]} {fmt(px,asset)} score:{score}')
...
def scan(asset):
    ...
    send_op(asset, direction, px, score)
...
def scanner_loop():
    log('SNIPER SCANNER v4.0 — score mínimo 80')
