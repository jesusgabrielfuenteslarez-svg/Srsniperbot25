...
def check_ops():
    ...
    for op in ops:
        cfg=ASSETS[op['a']]
        if now-op['t']>7200:
            log(f'{cfg["icon"]} Op #{op["n"]} cerrada por timeout')
            to_rm.append(op); continue
        p=get_spot(op['a'])
        if not p:
            log(f'{cfg["icon"]} Sin precio para seguimiento')
            continue
        hit_tp=(op['d']=='buy' and p>=op['tp']) or (op['d']=='sell' and p<=op['tp'])
        hit_sl=(op['d']=='buy' and p<=op['sl']) or (op['d']=='sell' and p>=op['sl'])
        log(f'{cfg["icon"]} Siguiendo #{op["n"]} | Precio:{fmt(p,op["a"])} TP:{fmt(op["tp"],op["a"])} SL:{fmt(op["sl"],op["a"])}')
        if hit_tp or hit_sl:
...
def señal_rapida(asset, direction):
    if asset not in ASSETS: return 'Activo no válido (gold/btc)', 400
    if direction not in ['buy','sell']: return 'Dirección no válida (buy/sell)', 400
    px=get_spot(asset)
    if not px:
        with lock: p=state['price'][asset]
        try: px=float(p.replace(',',''))
        except: return 'Sin precio disponible', 400
    send_signal(asset, direction, px, sc=99, manual=True)
    cfg=ASSETS[asset]
    return f'✅ Señal {direction.upper()} {cfg["label"]} enviada a Telegram — {fmt(px,asset)}', 200
...
def api_manual():
    ...
    if px<=0:
        px=get_spot(asset)
        if not px:
            # Usar último precio conocido del estado
            with lock:
                p=state['price'][asset]
            try:
                px=float(p.replace(',',''))
            except:
                return jsonify({'ok':False,'error':'No hay precio disponible — espera unos segundos'})
    send_signal(asset,direction,px,sc=99,manual=True)
    return jsonify({'ok':True})
...
def loop():
    ...
    while True:
        try:
            # Seguimiento TP/SL cada 10 segundos
            check_ops()

            # Escaneo cada ~25 segundos
            if cycle % 2 == 0:
