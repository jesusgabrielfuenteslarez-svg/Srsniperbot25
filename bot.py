...
# ── Umbrales de score ───────────────────────
SCORE_MIN       = 60   # mínimo para generar señal
...
def filtro_tendencia(c, h, l):
    """
    Tendencia puntúa pero NO bloquea.
    Un trader puede entrar contra tendencia si hay barrido + confirmación.
    Peso: 20 puntos
    """
    e20 = calc_ema(c, 20)
    e50 = calc_ema(c, 50)
    px  = c[-1]
    gap = abs(e20 - e50) / e50 * 100

    if e20 > e50 and px > e20:
        return 20 if gap > 0.05 else 12, 0
    if e20 < e50 and px < e20:
        return 0, 20 if gap > 0.05 else 12

    # Sin tendencia clara — puntos parciales en ambas direcciones
    # No bloquea, solo puntúa menos
    if px > e20: return 8, 0
    return 0, 8

def filtro_liquidez(c, h, l):
    """
    Barrido de máximos o mínimos.
    Ventana ampliada a 20 velas para detectar más setups.
    Peso: 25 puntos
    """
    if len(c) < 10: return 0, 0

    # Buscar en ventanas de 5, 10 y 20 velas
    for lookback in [5, 10, 20]:
        if len(h) < lookback + 1: continue
        prev_h = max(h[-lookback:-1])
        prev_l = min(l[-lookback:-1])
        rng    = h[-1] - l[-1]
        if rng == 0: continue

        if l[-1] < prev_l and c[-1] > prev_l:
            wick = (min(c[-2], c[-1]) - l[-1]) / rng
            if wick >= 0.25:
                return 25 if wick >= 0.50 else 18, 0

        if h[-1] > prev_h and c[-1] < prev_h:
            wick = (h[-1] - max(c[-2], c[-1])) / rng
            if wick >= 0.25:
                return 0, 25 if wick >= 0.50 else 18

    # Sin barrido claro — buscar rebote en soporte/resistencia
    if len(c) >= 20:
        mn = min(l[-20:]); mx = max(h[-20:])
        rng = mx - mn
        if rng > 0:
            pos = (c[-1] - mn) / rng
            if pos < 0.15: return 12, 0   # cerca del fondo del rango
            if pos > 0.85: return 0, 12   # cerca del techo

    return 0, 0

def filtro_confirmacion(c, h, l):
    """
    Vela de confirmación en la dirección correcta.
    Más flexible — acepta cuerpos desde 30%.
    Peso: 25 puntos
    """
    ...
    if c[-1] > c[-2]:
        if body_pct >= 0.60:
            ...
            pts = 0
        if pts > 0 and c[-1] > max(c[-4:-2]): pts = min(pts + 5, 25)
        return pts, 0
    ...
    if c[-1] < c[-2]:
        if body_pct >= 0.60:
            ...
            pts = 0
        if pts > 0 and c[-1] < min(c[-4:-2]): pts = min(pts + 5, 25)
        return 0, pts
...
def filtro_volumen(v):
    ...
    Sin datos de volumen — puntos neutros (no bloquea).
    Peso: 15 puntos
    """
    if not v or len(v) < 6: return 8, 8
    avg = sum(v[-7:-1]) / 6
    if avg == 0: return 8, 8
    ratio = v[-1] / avg
    if ratio >= 1.5:  return 15, 15
    if ratio >= 1.2:  return 12, 12
    if ratio >= 0.8:  return 8,  8
    return 4, 4   # volumen bajo pero no bloquea

def filtro_momentum(c, h, l):
    """
    Desplazamiento real. Mercado muerto = 0 puntos.
    Peso: 15 puntos
    """
    ...
    if ratio < 0.3: return 0, 0   # mercado completamente muerto

    move = c[-1] - c[-6]
    if move > at * 0.5:    return 15, 0
    if move > at * 0.2:    return 10, 0
    if move < -at * 0.5:   return 0, 15
    if move < -at * 0.2:   return 0, 10
    return 5, 5   # movimiento leve
...
def score_oportunidad(d5, d1h=None):
    ...
    log(f'  T:{tb1}/{ts1} L:{tb2}/{ts2} C:{tb3}/{ts3} V:{tb4}/{ts4} M:{tb5}/{ts5}')
    log(f'  Score → BUY:{score_buy} SELL:{score_sell}')

    # Solo necesita superar el score mínimo
...
<div class="page">
  ...
  <div class="card">
    <div class="lbl">📊 ORO — XAU/USD M5</div>
    <iframe src="https://s.tradingview.com/widgetembed/?frameElementId=tv1&symbol=OANDA%3AXAUUSD&interval=5&theme=dark&style=1&locale=es&toolbar_bg=%230b1219&hide_top_toolbar=0&studies=RSI%4014&withdateranges=0&hideideas=1"
      style="width:100%;height:280px;border:none;border-radius:6px" allowtransparency="true" scrolling="no"></iframe>
  </div>

  <div class="card">
    <div class="lbl">📊 BITCOIN — BTC/USD M5</div>
...
def scanner_loop():
    log('SNIPER SCANNER v4.1 — score mínimo 60')
