...
def send_signal(asset, direction, px, sc, manual=False):
    ...
    gains='\n'.join([f'{l:.2f} → +{gain(cfg["tp"],l)}€' for l in LOTS])
    with lock:
        state['op_n']+=1; state['sigs']+=1; n=state['op_n']
    send(f'{cfg["icon"]} {cfg["label"]}\n\n{tipo}\n\nConfianza: {sc}%\n\nEntrada: {fmt(px,asset)}\nTP: {fmt(tp,asset)}\nSL: {fmt(sl,asset)}\n\nGanancia estimada:\n{gains}\n\nHora: {now_str()}')
    ...
    with lock:
        ...
        state['last_signal']={'n':n,'a':asset,'d':direction,'px':fmt(px,asset),'tp':fmt(tp,asset),'sl':fmt(sl,asset),'sc':sc,'h':now_str(),'icon':cfg['icon'],'label':cfg['label']}
        state['ops'].append({'a':asset,'d':direction,'tp':tp,'sl':sl,'t':time.time(),'n':n,'s':sc})
