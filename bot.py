...
def analyze(asset, data):
    ...
    # CAPA 3: Liquidity Sweep (prioridad maxima)
    ...
    if sweep_dir:
        sig = sweep_dir  # sweep tiene prioridad
        bonus = 20       # bonus de puntuacion por sweep
    ...
    ctx     = '⚡ Sweep institucional' if sweep_dir else ' | '.join(reasons[:2])
