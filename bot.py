...
def hourly():
    global last_h_report
    n = datetime.now()
    if n.minute != 0 or n.hour == last_h_report: return
    last_h_report = n.hour
    ...
    h   = f'{(n.hour-1)%24:02d}:00'
    send(f'<b>RESUMEN {h}-{n.hour:02d}:00</b>\n'
         f'✅{tp} ❌{sl} 🔄{len(active_sigs)}\n'
         f'Efectividad: <b>{pct}%</b> {st}')
