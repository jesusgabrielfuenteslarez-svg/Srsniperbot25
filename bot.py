    ...
    for s in active_sigs:
        ...
        if hit_tp or hit_sl:
            ...
            if hit_tp:
                ...
                send(f'TP {cfg["icon"]} +{cfg["tp"]} | {fmt(s["e"])}→{fmt(p)} +{age}m\n'
                     f'{round(daily_pnl)}€ hoy | {pct}% ({tp_t}TP {sl_t}SL)')
            else:
                entry_count[s['asset']] = 0
                send(f'SL {cfg["icon"]} -{cfg["sl"]} | {fmt(s["e"])}→{fmt(p)} +{age}m\n'
                     f'{round(daily_pnl)}€ hoy | {pct}% ({tp_t}TP {sl_t}SL)')
...
# =============================================
# MONITOR SILENCIO — avisa si 2h sin señal
# =============================================

def check_silence():
    global last_silence
    now = time.time()
    if now - last_sig_time < SILENCE_LIMIT:
        return
    if now - last_silence < 1800:
        return
    last_silence = now
    sess_idx, sess_name = current_session()
    if day_stopped:
        motivo = 'objetivo diario alcanzado'
    elif survival_mode:
        motivo = 'survival mode activo'
    elif news_blocked():
        motivo = 'pausa por noticias'
    elif sess_idx is None:
        motivo = 'fuera de sesion — esperando setup'
    else:
        motivo = f'en {sess_name} — sin confluencia suficiente'
    send(f'BOT ACTIVO — sin señales 2h\nMotivo: {motivo}\n{now_str()}')
...
def daily_report():
    ...
    send(f'RESUMEN {n.strftime("%d/%m")}\n'
         f'TP:{tp} SL:{tot-tp} | {pct}% {st}\n'
         f'PnL: {round(daily_pnl)}EUR\n'
         f'ORO {tg}TP {sg}SL | BTC {tb}TP {sb}SL')
    for k in stats:
...
def analyze(asset, data):
    ...
    gains = ' | '.join([f'{l:.2f}+{GAINS[asset][l]:.0f}€' for l in [0.05, 0.10, 0.15]])
    ...
    send(
        f'{tipo} {cfg["icon"]} {score}pts{e_lbl}\n'
        f'📍{fmt(px)} 🎯{fmt(tp)} 🛑{fmt(sl)}\n'
        f'{ctx} | {ico}{rec}\n'
        f'{gains}\n'
        f'{round(daily_pnl)}€ hoy | {pct_t}% | {sess_name} {now_str()}'
    )
