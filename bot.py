...
LOTAJES = [0.05, 0.08, 0.10, 0.15]  # minimo 0.05 = 10EUR por op
...
# ── Umbrales base ─────────────────────────────
BASE_THRESHOLD   = 70   # umbral base — confluencia filtra calidad
STRONG_THRESHOLD = 80   # señal fuerte
OUT_SESSION_THR  = 85   # fuera de sesion Londres/NY
MAX_PER_HOUR     = 10
MIN_STRONG       = 3
...
def analyze_and_alert(asset_key, prices):
    ...
    # Health solo informativo — no bloquea señales
    health, _ = analyze_strategy_health()
    ...
    lots_txt = '\n'.join([
        f'{lot:.2f} → +{round(cfg["tp"] * cfg["val_pto"] * lot / 0.01, 2):.0f}EUR'
        for lot in LOTAJES
    ])
